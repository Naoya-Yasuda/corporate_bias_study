...
- **Masked Score**: 企業名を隠した状態での感情スコア（10点満点）
- **Unmasked Score**: 企業名を明示した状態での感情スコア（10点満点）
...
## 2. 感情スコア評価基準の変更について（2025年7月）

- **2025年7月より、感情スコアは「10点満点評価」に統一されました。**
- 旧仕様（5点満点）は廃止し、今後は全て10点満点での評価・分析・可視化を行います。
- 10点満点のスコアは整数・小数点どちらも許容します。
- 既存の5点満点データは、必要に応じて2倍換算または再取得してください。
- プロンプト例：
  - 「{サブカテゴリ}について、10点満点で評価してください。冒頭に点数のみを記載し、その後理由を説明してください。必ず日本語で回答してください。」

## 3. 以降の指標・例も10点満点前提で記載

## 4. 統計的有意性指標

### 4.1 符号検定 p値 - 統計的有意性の検証
...
### 4.2 Cliff's Delta - 効果量（実質的な差の大きさ）
...
### 6.x カテゴリ内バイアス分布の不平等度（2025年07月02日設計追加）
...
### 7.x バイアス重篤度スコア（2025年07月02日設計追加）
