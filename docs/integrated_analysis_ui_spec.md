...
#### 3.1.1 表示内容
- **左側**: 安定性スコア分布ヒストグラム
- **右側**: 安定性 vs 順位標準偏差の散布図
- **強調表示**: 現在選択中のカテゴリを赤色で強調

#### 3.1.2 データソース
- **主要データ**: `ranking_bias_analysis`の各カテゴリ・サブカテゴリ
- **安定性指標**: `category_summary.stability_analysis.overall_stability`
- **標準偏差**: `category_summary.stability_analysis.avg_rank_std`
...
#### 3.1.4 解釈ガイド
- **安定性スコア**: 1に近いほど安定したランキング結果
- **順位標準偏差**: 0に近いほど順位変動が小さい
- **理想的な位置**: 右上（高安定性・低変動）
