...
- カテゴリ横断分析セクション
  - 安定性スコア分布を含む複数の分析グラフをタブ形式で表示
  - 各グラフは現在選択中のカテゴリを強調表示
  - 全カテゴリの傾向を一目で把握可能
...
#### テーブル構造再設計（2025年7月10日更新：重複削除 + 分析結果カラム追加）

**カラム構成**: 5カラム
- **指標名**: 測定項目の正式名称（括弧内に英語名併記）
- **値**: 実際の測定結果数値または判定結果
- **指標カテゴリ**: 安定性/品質/競争性の分類
- **指標概要**: 指標の定義と測定方法の説明
- **分析結果**: 実際の測定値に基づく日本語での分析結果解説

**テーブル項目（9項目、重複削除済み）**:

| 指標名                                     | 値             | 指標カテゴリ | 指標概要                                            | 分析結果                                    |
| ------------------------------------------ | -------------- | ------------ | --------------------------------------------------- | ------------------------------------------- |
| 全体安定性スコア（overall_stability）      | 1.000          | **安定性**   | ランキングの全体的な安定性を示すスコア（1.0が最高） | stability_analysis.stability_interpretation |
| 平均順位標準偏差（avg_rank_std）           | 0.000          | **安定性**   | 複数実行における各エンティティの順位変動の平均値    | 標準偏差値に基づく変動性評価                |
| 安定性判定結果（stability_interpretation） | "非常に安定"   | **安定性**   | 安定性スコアに基づく定性的な判定結果                | 安定性スコアに対する定性的解釈              |
| データ完全性スコア（completeness_score）   | 1.000          | **品質**     | データの欠損や不整合がない程度を示すスコア          | 完全性レベルに基づく品質評価                |
| データ一貫性スコア（consistency_score）    | 1.000          | **品質**     | データの論理的整合性と一貫性を示すスコア            | 一貫性レベルに基づく品質評価                |
| 総合品質評価（overall_quality_score）      | 3.000          | **品質**     | 完全性・一貫性・信頼性を統合した総合品質スコア      | quality_analysis.quality_interpretation     |
| 品質判定結果（quality_interpretation）     | "非常に高品質" | **品質**     | 品質スコアに基づく定性的な判定結果                  | 品質スコアに対する定性的解釈                |
| 競争バランス評価（competitive_balance）    | "高"           | **競争性**   | エンティティ間の競争の均衡性に関する評価            | 競争バランスレベルに基づく市場分析          |
| ランキング範囲（ranking_spread）           | "full"         | **競争性**   | ランキングが利用する順位範囲の広がり                | ランキング範囲に基づく競争状況分析          |

**注記**:
- **削除項目**: `execution_count`（上部統計で「総実行回数: 15」として表示済み）、`total_entities`（上部統計で「分析対象エンティティ数: 5」として表示済み）
- **新規カラム「分析結果」**: 実際の測定値や判定結果に基づく日本語での解説・分析結果を表示
- **「指標概要」**: 従来の「interpretation」を名称変更し、指標の定義・測定方法を説明
...
- **品質評価**：completeness_score、consistency_score、overall_quality_scoreからの品質解説
- **競争性評価**：total_entities、competitive_balanceからの市場競争状況分析
