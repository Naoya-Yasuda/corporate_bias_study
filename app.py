...
if viz_type == "時系列分析":
    ...
    # OKなデータだけを対象にする
    ok_dates = [date for date, (data, _) in best_data_by_date.items() if data is not None]
    ok_dates_sorted = sorted(ok_dates, reverse=True)
    if period_n is not None:
        selected_dates = sorted(ok_dates_sorted[:period_n], reverse=False)
    else:
        selected_dates = sorted(ok_dates)
    ...
    # --- タブUIでグラフ種別を切り替え ---
    tabs = st.tabs(["BI値時系列", "重篤度スコア時系列", "p値時系列", "効果量時系列"])
    with tabs[0]:
    ...
    with tabs[1]:
        st.info("重篤度スコアの時系列グラフは今後実装予定です。")
    with tabs[2]:
        st.info("p値の時系列グラフは今後実装予定です。")
    with tabs[3]:
        st.info("効果量の時系列グラフは今後実装予定です。")
...
"""
            ...
            # --- 複数日付データ取得 ---
            dashboard_data_by_date = {}
            for date in selected_dates:
                dashboard_data = loader.get_integrated_dashboard_data(date)
                if dashboard_data:
                    dashboard_data_by_date[date] = dashboard_data
            if not dashboard_data_by_date:
                st.sidebar.error("選択された日付の分析データが見つかりません")
                st.stop()

            # --- カテゴリ・サブカテゴリ選択 ---
            # 代表日付（最新）からカテゴリリストを取得
            latest_date = max(dashboard_data_by_date.keys())
            analysis_data = dashboard_data_by_date[latest_date]["analysis_results"]
            sentiment_data = analysis_data.get("sentiment_bias_analysis", {})
            all_categories = [c for c in sentiment_data.keys() if c not in ("全体", "all", "ALL", "All")]
            all_categories.sort()
            selected_category = st.sidebar.selectbox(
